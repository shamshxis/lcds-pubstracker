...
with st.sidebar:
    ...
    elif "Year" in period: start = now - timedelta(days=365)
...
with tab1:
    ...
    view = df_filt.copy()
    if search: view = view[view['Title'].str.lower().contains(search, na=False) | view['LCDS Author'].str.lower().contains(search, na=False)]
    ...
    st.dataframe(view[['Date','Citations','LCDS Author','Title','Journal']], use_container_width=True, hide_index=True)
...
with tab2:
    ...
    with c2:
        ...
        if not df_filt.empty:
            jour = df_filt.groupby('Journal')['Citations'].sum().sort_values(ascending=False).head(10).reset_index()
